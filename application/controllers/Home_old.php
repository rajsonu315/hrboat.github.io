  ...
  public function __construct() {
		...
		public function feedback()
	{
		...
		if($check->num_rows() > 0){
			...
			$replay = '<p>Please slect your age</p>
					<button type="button" class="18 removebtn" id="18">18-24 years</button>
					<button type="button" class="25 removebtn" id="25">25-34 years</button>
					<button type="button" class="35 removebtn" id="35">35-44 years</button>
					  <button type="button" class="45 removebtn" id="45">45-54 years</button>
					  <button type="button" class="55 removebtn" id="55">55 years or more</button>'
					;
	...
	public function home()
	{
		...
		if($check->num_rows() > 0){
			...
			$replay = '<p>Hi '.$fetch_data["name"].', what can we help you with today?</p>
						<button type="button" class="removebtn feedback" id="feedback">What is your age</button>
					  <button type="button" class="removebtn program_faq" id="program_faq">Please tell me which of these do you own? It could be owned by you or your family or provided by the employer or it could be available in the house you live in; but it should be for the use of just you or your family.</button>
					  <button type="button" class="removebtn ott" id="ott">Where do you usually watch the content on these OTT platforms</button>';
	...
	public function program_faq()
	{
		...
		if($check->num_rows() > 0){
			$fetch_data = $check->row_array();
			$replay = '<p>Please tell me which of these do you own? It could be owned by you or your family or provided by the employer or it could be available in the house you live in; but it should be for the use of just you or your family. </p>
					<button type="button" class="what_src removebtn" id="1">Electricity Connection</button>
					  <button type="button" class="2 removebtn" id="2">Air-conditioner </button>
					  <button type="button" class="3 removebtn" id="3">4 Wheeler: Car, Jeep etc </button>
					  <button type="button" class="4 removebtn" id="4">Washing Machine </button>
					  <button type="button" class="5 removebtn" id="5">Refrigerator</button>
					  <button type="button" class="6 removebtn" id="6">Colour Television</button>
					  <button type="button" class="7 removebtn" id="7">2 Wheeler: Scooter, Bike etc</button>
					  <button type="button" class="8 removebtn" id="8">Laptop/ PC </button>
					  <button type="button" class="9 removebtn" id="9">LPG Stove</button>
					  <button type="button" class="10 removebtn" id="10">Ceiling Fan</button>
					  <button type="button" class="11 removebtn" id="11">Agricultural land in this city or anywhere else</button>'
		...
		public function ottfd()
	{
		...
		if($check->num_rows() > 0){
			$data = $this->input->post();
			/* $replay = 'thanks for valueable feedback'; */
			
				$replay = "<p>How likely are OTT platforms.</p>
							<button type='button' class='ot1 removebtn' id='ot1'>Mostly on TV</button>
							<button type='button' class='ot2 removebtn' id='ot2'>Mostly on Mobile </button>
							<button type='button' class='ot3 removebtn' id='ot3'>Mostly on PC / Laptop</button>
							<button type='button' class='ot4 removebtn' id='ot4'>Any available device</button>
						";
			}
		...
		public function feedback_store77()
	{
		$nm = isset($_REQUEST['payload'])?$_REQUEST['payload']:0;
		$payload = ($this->session->userdata('payload'))?$this->session->userdata('payload'):$nm;
		$check = $this->db->order_by('id','desc')->get_where('stellar_international_consumer',array('id' => $payload));
		if($check->num_rows() > 0){
			...
			public function feedback_store3()
	{
		$nm = isset($_REQUEST['payload'])?$_REQUEST['payload']:0;
		$payload = ($this->session->userdata('payload'))?$this->session->userdata('payload'):$nm;
		$check = $this->db->order_by('id','desc')->get_where('stellar_international_consumer',array('id' => $payload));
		if($check->num_rows() > 0){
			...
			$replay = '<p>Where do you usually watch the content on these OTT platforms? </p>
			  <button type="button" class="removebtn ott" id="program_faq">Where do you usually watch the content on these OTT platforms? </button>';
			echo $replay;
	...
	public function feedback_store()
	{
		...
		if($check->num_rows() > 0){
			...
			$arr = array('feedback' => $data['text'],'consumer_id' => $payload);
			$this->db->insert('consumer_feedback',$arr);
			$replay = '<p>Please click next question.</p>
			  <button type="button" class="removebtn program_faq" id="program_faq">Please tell me which of these do you own? It could be owned by you or your family or provided by the employer or it could be available in the house you live in; but it should be for the use of just you or your family.</button>';
			echo $replay;
		...
		public function feedback_store2()
	{
		$nm = isset($_REQUEST['payload'])?$_REQUEST['payload']:0;
		$payload = ($this->session->userdata('payload'))?$this->session->userdata('payload'):$nm;
		$check = $this->db->order_by('id','desc')->get_where('stellar_international_consumer',array('id' => $payload));
